...
class Dictionary(object):
    ...
    def tokenize(self, sentence, add_word):
        ...
        sentence = sentence.replace(',', '').replace('?', '').replace('\'s', ' \'s').replace('-',
                   ' ').replace('.','').replace('"', '').replace('n\'t', ' not').replace('$', ' dollar ') # this has been edited
        ...
        if add_word:
            ...
            for w in words:
                ...
                tokens.append(len(self.word2idx)) # this has been added
        return tokens
...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return features, question, target, target_margin, bias, question_id, freq_margin0, q_type, image_id, spatials
